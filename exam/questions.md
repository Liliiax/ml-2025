...
25. [Безградиентные методы оптимизации](#25-безградиентные-методы-оптимизации)
...
* Связь нелинейная: Если истинная зависимость сложная (квадратичная, периодическая, экспоненциальная), линейная модель даст очень плохие предсказания.
* Разброс ошибок не постоянен / Ошибки коррелируют между собой
* Наличие выбросов: Линейная регрессия очень чувствительна к выбросам, так как минимизирует квадрат ошибки. Один выброс может сильно исказить линию.
* Мультиколлинеарность: Высокая корреляция между независимыми переменными делает оценки коэффициентов неустойчивыми и трудными для интерпретации.
* Задача классификации: Когда целевая переменная категориальная (да/нет, кошка/собака). Прямое применение линейной регрессии даст бессмысленные предсказания (


## 4. Процесс минимизации квадратичного функционала эмпирического риска
[*Вернуться к содержанию*](#содержание)
> Функция потерь (loss function) — отображение результата работы алгоритма на R, показывающее "стоимость" ошибки.

<img src="https://github.com/Liliiax/ml-2025/blob/03a9529effc2085eb6e136c6d65b4666379495f3/exam/src/q4.png" width="80%"/>

### Метод минимизации эмпирического риска
Логично предположить, что если алгоритм хорошо показывает себя на обучающей выборке, то и на реальных данных он будет работать неплохо. Так подходим к конструктивному методу обучения — _методу минимизации эмпирического риска_. Суть метода, как следует из названия, в минимизации функционала 
$$Q(a,X_m)$$:

$$a^∗=argmin_{a \in A} Q(a,X_m)$$

* Метод простой, общий, конструктивный и зачастую сводит задачу обучения к численному поиску минимума в модели алгоритма. Однако столь пристальное внимание к обучающей выборке приводит к явлению _**переобучения**_.

### Есть два основных класса методов для нахождения минимума этого функционала:

* Аналитическое решение (для линейной регрессии с MSE)
> В случае линейной модели и квадратичной ошибки можно найти минимум точно, взяв производную по вектору параметров w, приравняв ее к нулю и решив получившуюся систему уравнений.
> **Плюсы**: Точное, быстрое решение в одну операцию.
> **Минусы**: Вычислительно тяжело для очень больших X (инверсия матрицы XᵀX имеет сложность ~O(n³)), неприменимо для сложных нелинейных моделей.

* Итеративные методы (Градиентный спуск и его варианты)
> Идея: Мы идем в направлении, противоположное градиенту функционала по параметрам. Градиент указывает направление наискорейшего роста функции, значит, антиградиент — направление наискорейшего спада.

`точно сходится с линейной скоростью для сильно выпуклых и L-гладких функций ошибки!!!`

### Градиентный спуск:
> η — скорость обучения (learning rate), гиперпараметр, определяющий размер шага.
> оптимальный learning rate: M/2L^2 ( M и L - из определений выпуклости и L-гладкости)

* _Инициализация_: Начальные значения параметров w (случайные или нулевые).
* _Итерация_: Вычислить градиент эмпирического риска на всей обучающей выборке: `∇R(w) = (1/N) Σ ∇L(y_i, h_w(x_i))`
			  Сделать шаг в сторону антиградиента: w_new = w_old - η * ∇R(w_old).
* _Критерий остановки_: Повторять шаг 2, пока функционал не перестанет значительно уменьшаться или не будет достигнуто заданное число итераций.

#### Варианты:

1. Стохастический градиентный спуск (SGD): Вместо градиента по всей выборке (N примеров) вычисляют градиент по одному случайному примеру. Это намного быстрее на одной итерации, но путь к минимуму становится более "шумным". Это помогает выпрыгивать из локальных минимумов.

2. Mini-batch SGD ( хз вроде не было у нас такого): Градиент вычисляется на небольшой случайной подвыборке данных (батче, например, из 32 или 128 примеров). Это самый популярный метод в глубоком обучении. Он сочетает вычислительную эффективность и устойчивость оценки градиента.

### Метод Ньютона-Рафсона 

Метод локально аппроксимирует функцию потерь квадратичной функцией, вычисляет минимум этой квадратичной аппроксимации и итеративно уточняет его до сходимости. Шаг обучения настраивается автоматически по скорости изменения градиента домножением на матрицу Гессе. Домножение на матрицу подстраивает скорость изменения вдоль каждой оси, используя градиенты вдоль всех осей. 

Достоинство метода — быстрая сходимость в окрестности точки экстремума. Недостатки — необходимость, помимо градиента, на каждой итерации вычислять обратную матрицу Гессе и зависимость сходимости метода от выбора начальной точки.

Метод Ньютона-Рафсона с матрицей Гессе применяется, когда целевая функция непрерывна и дважды дифференцируема. Условие сходимости — матрица Гессе должна быть положительно определённой.

### Метод Левенберга-Марквардта 

сложно, кажется это когда вместо матрицы Гессе берем только ее диагональку
...
$$ \text{SSE}(y, \hat y) =  \sum_{i=1}^N (y_i - \hat y_i)^2$$
...
> **Сингулярное разложение матрицы признаков** (SVD) — метод факторизации матриц, который используется в машинном обучении для понижения размерности данных и извлечения признаков. 


#### Суть: матрицу признаков представляют в виде произведения трёх других матриц: 
* `U` — матрица левых сингулярных векторов, содержит информацию о взаимосвязи между признаками;
* `S` — диагональная матрица сингулярных значений, отражает значение каждого признака;
* `V` — матрица правых сингулярных векторов, содержит информацию о взаимосвязи между объектами данных.

### Принцип работы
`SVD` позволяет разложить любую матрицу на последовательность простых линейных преобразований: поворот (или отражение), масштабирование вдоль новых координатных осей и обратный поворот. Благодаря этому разложение даёт геометрическую интерпретацию действия матрицы на пространство: каждое сингулярное число определяет, насколько сильно матрица растягивает пространство вдоль соответствующего направления. 

### Зачем SVD нужно? 
1. **Понижение размерности**
> Мы можем аппроксимировать исходную матрицу A, используя только r самых больших сингулярных чисел. Усеченное SVD (Truncated SVD):
$$A \approx A_r = U_r \Sigma_r V_r^T$$, где $$U_r, \Sigma_r, V_r$$ содержат только первые r столбцов/строк.
> Почему это работает? SVD автоматически находит направления максимальной дисперсии в данных. Первые сингулярные векторы (v₁, v₂, ...) — это главные компоненты данных (связь с PCA). Отбрасывая компоненты с малыми $$\sigma_i$$, мы отбрасываем "шум" и маловажную информацию.

#### **Решение задач линейной регрессии**
> При решении линейной регрессии через нормальные уравнения мы сталкиваемся с матрицей XᵀX. Она может быть _плохо обусловленной_. Ее обращение численно неустойчиво, маленькие ошибки в данных приводят к огромным изменениям в весах w. Может быть вырожденной и тогда (XᵀX)⁻¹ не существует.

`Решение через SVD`:
* Разложим матрицу признаков: X = U Σ Vᵀ
* Псевдообратная матрица $$X^{+}$$ вычисляется легко: $$X^{+} = V \Sigma^{+} U^T$$
* Где $$\Sigma^{+}$$ — диагональная матрица, в которой каждый ненулевой диагональный элемент $$\sigma_i $$ заменяется на $$\frac{1}{\sigma_i}$$, а нулевые остаются нулями.

Решение линейной регрессии: $$w = X^{+}y=V \Sigma^{+} U^T y$$

**Преимущества:** `стабильность, работа с вырожденными случаями, интерпретируемость` 

3. **Вычисление собственных векторов и PCA**
Главные компоненты (PCA) — это собственные векторы ковариационной матрицы XᵀX (для центрированного X). Из SVD следует:
Правые сингулярные векторы V матрицы центрированного X — это и есть главные компоненты.
На практике PCA всегда вычисляют через SVD, а не через собственные значения XᵀX, так как это численно стабильнее и эффективнее.
...
[*Вернуться к содержанию*](#содержание)
а о ну это кажется в вопросе 9 фактически + еще регуляриpация полезна : $$w_\lambda = (X^TX + \lambda E)^{-1} X^T y$$

 [Решение задач линейной регрессии](#решение-задач-линейной-регрессии)
...
1. Инициализация — задаётся начальная точка.
2. Вычисление направления спуска в текущей точке. Например, в градиентных методах в качестве направления спуска выбирается антиградиент функции в точке.
3. Определение шага — подбирается шаг, двигаясь с которым вдоль направления спуска, получается новая точка с меньшим значением целевой функции. Шаг может быть фиксированным (постоянным на всех итерациях) или адаптивным (вычисляемым для минимизации функции вдоль направления спуска).
4. Обновление точки — переход к новой точке.
5. Проверка остановки — алгоритм прекращается, если выполнено условие сходимости.

### Критерии остановки процесса:
1. Изменение функции потерь — если её значения перестают существенно уменьшаться, алгоритм можно остановить.
2. Выполнение условия сходимости — например, норма градиента в текущей точке не становится меньше заданного порога.
3. Задание максимально возможного числа итераций — обычно правила остановки дополняются этим условием, чтобы избежать преждевременной остановки
   
## 13. Оценка сходимости градиентного спуска для гладких выпуклых функций
[*Вернуться к содержанию*](#содержание)

ну там кринж математика хз

Для такой функции: Grad(f(x))=0 равносильно x=argmin [f(x)] $$x \in X^m$$ сойдется

при шаге $$h_k \in (0; \frac{M}{L^2})$$ сходится ( а дипсик гвоорит что при таком `h ∈ (0, 2/L)` и говорит что то неверно)


## 14. Назначение и реализация стохастического градиентного спуска
[*Вернуться к содержанию*](#содержание)
В стохастическом («онлайновом») градиентном спуске истинный градиент Q(w) аппроксимируется градиентом одного тренировочного примера: 
$$\displaystyle w:=w-\eta \nabla Q_{i}(w)$$

Пробегая через тренировочное множество, алгоритм осуществляет приведённый выше пересчёт для каждого тренировочного примера. Для достижения сходимости алгоритма может потребоваться несколько проходов по тренировочному набору данных. Перед каждым новым проходом данные в наборе перемешиваются для устранения возможности зацикливания алгоритма. Типичные реализации могут использовать скорость обучения для улучшения сходимости.

В псевдокоде стохастический градиентный спуск можно представить следующим образом:

1. Выбрать начальный вектор параметров w и скорость обучения $$\displaystyle \eta$$
2. Повторять до достижения приблизительного минимума
3. Случайным образом перемешать примеры в тренировочном множестве.
4. Для $$\displaystyle i=1,2,...,n$$ выполнить $$\displaystyle \!w:=w-\eta \nabla Q_{i}(w)$$
...
* Быстрое обучение
* Регуляризация: Легко добавить L1/L2 для отбора признаков
